    ...
    private async processImage(message: Message): Promise<{ description: string } | null> {
        try {
            if ("photo" in message && message.photo?.length > 0) {
                ...
                console.log("Service obtained:", !!imageDescriptionService);

                try {
                    // Get the image description
                    const { title, description } = await imageDescriptionService.describeImage(imageData, 'image/jpeg', userCaption);

                    // Return just the analysis without the "User Query" prefix
                return {
                    description: `${title}\n${description}`
                };
        ...
        try {
            ...
            if ("text" in message) {
                ...
                messageText = message.caption;
                    ...
                    // For image messages, send the response directly
        if (imageInfo && imageInfo.description) {
            await ctx.reply(imageInfo.description);
            return;  // Return early for image processing
        }
            ...
            // Create content object with the combined text
            ...
            // Update state and check if should respond
            ...
            if (shouldRespond) {
                ...
                if (responseContent && responseContent.text) {
                    await this.sendMessageInChunks(ctx, responseContent, message.message_id);
                    ...
                    if (responseContent) {
                        await this.runtime.processActions(memory, [memory], state, async (content: Content) => {
            ...
            // Skip evaluation for image messages
            if (!("photo" in message)) {
                await this.runtime.evaluate(memory, state, shouldRespond, async (content: Content) => {
                    const sentMessages = await this.sendMessageInChunks(ctx, content, message.message_id);
                    return sentMessages || [];
                });
            }
